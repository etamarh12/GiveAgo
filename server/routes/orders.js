...
import { newOrder, delOrder, allOrders, comDelivery, takeDelivery,filteredOrders } from "../controllers/orders.js";
...
router.post('/filteredOrders', async (req, res) => {
  try {
    const response = await filteredOrders(req, res);
    console.log("routes succes : " + res)
    return res.status(200);
  } catch (error) {
    console.log("routes eror : " + error)
